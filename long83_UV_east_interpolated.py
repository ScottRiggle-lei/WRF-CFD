...
if __name__ == '__main__':
    ...
    for t in range(num_time_steps):
        ...
        znu = data.variables['ZNU'][0, :size_h]
       ...
       # omg_data=(ust_expanded**3)/(0.0417*k_data*0.41*(heights_ti+0.03))
        ...
        for h in range(size_h):
            ...
            lat_y_U.extend(u_data[h, (size_s - size_s):(size_n-size_s+2), size_e - size_w+1])
            lat_y_V.extend(v_data[h, (size_s - size_s):(size_n-size_s+2), size_e - size_w+1])
            lat_y_W.extend(w_data[h, (size_s - size_s):(size_n-size_s+2), size_e - size_w+1 ])  # 取反
            lat_Z.extend(heights_ti[h, (size_s - size_s):(size_n-size_s+2), size_e - size_w+1])
            lat_y_UST.extend(ust_expanded[h, (size_s - size_s):(size_n-size_s+2), size_e - size_w+1 ])
            longitudes_y.extend((xlat_data[(size_s - size_s):(size_n-size_s+2), size_e - size_w+1 ] - xlat_data[size_s - size_s+1, size_e - size_w+1 ]) * lat_r + 0)
            lat_y_K.extend(k_data[h, (size_s - size_s ):(size_n - size_s + 2), size_e - size_w+1])
            lat_y_OMG.extend(omg_data[h, (size_s - size_s ):(size_n - size_s + 2), size_e - size_w+1])
            # omg_values = omg_data[h, (size_s - size_s + 1):(size_n - size_s + 1), size_e - size_w]
    ...
    # 保存插值后的结果到文件
    interpolated_df.to_csv('F:/wenjian/WRF_CFD/02-shezhen-2022/01-Shenzhen-WRF/2022021911-2022022017/wangqiang/lath_east_UV20220219.txt', index=False, header=False, sep='\t', float_format='%.5f')
