                ...
                for j in range(1, num_points + 1):  # Exclude the first point to avoid duplicate
                    interpolated_data.append(
                        [row1['Repeated_time'], interp_heights[j], interp_x_coords[j], interp_wind_Uspeeds[j],
                         interp_wind_Vspeeds[j], interp_wind_Wspeeds[j], interp_wind_UST[j], interp_wind_K[j],
                         interp_wind_OMG[j]])
...
if __name__ == '__main__':
    ...
    # 计算高度
    height_z = calculate_heights(data, size_h, size_e-size_s)
    ...
    for t in range(num_time_steps):
        ...
        for h in range(size_h):
            ...
            lat_y_U.extend(u_data[h, size_s - size_s, :size_e - size_w + 2])  # 取反
            lat_y_V.extend(v_data[h, size_s - size_s, :size_e - size_w + 2])  # 取反
            lat_y_W.extend(w_data[h, size_s - size_s, :size_e - size_w + 2])  # 取反
            lat_y_UST.extend(ust_expanded[h, size_s - size_s, :size_e - size_w +2])  # 取
            lat_Z.extend(heights_ti[h, size_s - size_s, :size_e - size_w + 2])
            longitudes_x.extend((xlong_data[size_s-size_s, :size_e-size_w+2] - xlong_data[size_s-size_s, size_w-size_w+1]) * long_r)
            lat_y_K.extend(k_data[h, size_s - size_s, :size_e - size_w + 2])
            lat_y_OMG.extend(omg_data[h, size_s - size_s, :size_e - size_w + 2])
            # omg_values = omg_data[h, size_s - size_s, :size_e - size_w + 1]
    ...
    # 保存插值后的结果到文件
    interpolated_df.to_csv('F:/wenjian/WRF_CFD/02-shezhen-2022/01-Shenzhen-WRF/2022021911-2022022017/wangqiang/lath_south_UV20220219.txt', index=False, header=False, sep='\t', float_format='%.5f')
    # 关闭 NetCDF 文件
    data.close()
