...
def calculate_heights(data, size_h, size_e):
    ...
    psfc = data.variables['PSFC'][:, size_s:size_e, size_s:size_e]  # 地面气压
    hgt = data.variables['HGT'][:, size_s:size_e, size_s:size_e]  # 地表高度
    ptop = data.variables['P_TOP'][0]  # P_TOP变量
    u_shape = data.variables['U'][:, :size_h, size_s:size_e, size_s:size_e].shape
...
if __name__ == '__main__':
    # 读取 NetCDF 数据
    nc_file = 'F:/wenjian/WRF_CFD/02-shezhen-2022/01-Shenzhen-WRF/2022021911-2022022017/wangqiang/wrfout_d03_2022-02-19_00_00_00.nc'  # 你的 NetCDF 文件路径
    data = netCDF4.Dataset(nc_file, 'r')

    # 计算高度
    height_z = calculate_heights(data, size_h, size_e-size_s+1)
    ...
    for t in range(num_time_steps):
        ...
        xlat_data = data.variables['XLAT'][t, size_s - 1:size_n+2, size_w - 1:size_e + 2]
        k_data = np.where(
        ...
        for h in range(size_h):
            time.extend([t] * (size_n-size_s+2))
            lat_y_U.extend(u_data[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w])
            lat_y_V.extend(v_data[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w])
            lat_y_W.extend(w_data[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w])  # 取反
            lat_Z.extend(heights_ti[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w ])
            lat_y_UST.extend(ust_expanded[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w ])
            longitudes_y.extend((xlat_data[(size_s - size_s ):(size_n-size_s+2), size_w - size_w ] - xlat_data[size_s - size_s+1, size_w - size_w] ) * lat_r + 0)
            lat_y_K.extend(k_data[h, (size_s - size_s):(size_n-size_s+2), size_w - size_w ])
            lat_y_OMG.extend(omg_data[h, (size_s - size_s ):(size_n - size_s + 2), size_w - size_w])
            # omg_values = omg_data[h, (size_s - size_s + 1):(size_n - size_s + 1), size_w - size_w]
    ...
    print(f'Length of longitudes_y: {len(longitudes_y)}')
    print(f'Length of lat_y_U: {len(lat_y_U)}')
    print(f'Length of lat_y_V: {len(lat_y_V)}')
    print(f'Length of lat_y_W: {len(lat_y_W)}')
    print(f'Length of lat_y_UST: {len(lat_y_UST)}')
    ...
    # 保存插值后的结果到文件
    interpolated_df.to_csv('F:/wenjian/WRF_CFD/02-shezhen-2022/01-Shenzhen-WRF/2022021911-2022022017/wangqiang/lath_west_UV20220219.txt', index=False, header=False, sep='\t', float_format='%.5f')
    # 关闭 NetCDF 文件
    data.close()
